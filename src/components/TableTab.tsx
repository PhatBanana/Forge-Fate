...
export function TableTab({
  ...
  */
   ...
   */
  /* §115: the swing itself lives in `fightStrike.ts` now - every rule an
     attack asks before a die is thrown. What stays here is this render's
     context: the map it is traced across, how dark each square is, and
     whether this table counts high ground. */
  const strikeContext: StrikeContext = { sight: sightContext, litAt, houseRules };
  const strikesInto = (
    updated: Roster,
  ...
  ) => strikeInto(fight, strikeContext, updated, who, strikes, targetRef, opts);
  ...
  */
  ...
  const spendReactionOf = (target: Roster, c: Combatant) => strikeSpendReaction(target, c);

  /** Everything they can swing, either side of the table, for the questions
      that are about reach rather than about choosing. */
  const allStrikesFor = (c: Combatant) => strikeAllFor(fight, c, strikesFor);
   ...
   */
  const opportunitySwing = (c: Combatant) => strikeOpportunity(fight, c, strikesFor);
